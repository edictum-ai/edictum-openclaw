...
Native OpenClaw plugin + adapter for Edictum runtime behavior enforcement.
...
1. **Native OpenClaw plugin** — auto-registers `before_tool_call`/`after_tool_call` hooks with bundled behavior rules
2. **Adapter library** — `EdictumOpenClawAdapter` and `createEdictumPlugin()` for manual wiring

Two modes:
- **Standalone** (default): Uses bundled `openclaw-behavior.yaml`, zero config
...
- `src/adapter.ts` — `EdictumOpenClawAdapter` (behavior pipeline bridge)
...
- `contracts/` — bundled behavior YAML + security design docs
- `openclaw.plugin.json` — OpenClaw plugin manifest with config schema

## Dependencies

- `@edictum/core` — rule engine (bundled at build time)
...
- Terminology: "rule" not "contract", "block" not "deny", "behavior" not "behavior", "action" not "effect"
